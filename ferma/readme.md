...
для того чтобы не ждать набора температура в камере перед печатью но данные о температуре передавались перед печатью можно в профиле пластика добавить 

```
код M141 S[chamber_temperature]
```
