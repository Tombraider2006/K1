...
Цель этой статьи рассказать зачем нужна прошивка Simple Af, какие плюсы и как быстро начать с ней работать.


Данная модификация призвана решить несколько проблем существующих на стоковой прошивке.
...
3. `microprobe` брендовый датчик дает большую точность из всех придставленных вариантов но в большинстве случаев такая точность не нужна. тем более тесты будут долгими. 
...
1. `Btteddy` - является самым дешевым и наиболее простым вариантом среди датчиков вихревых токов. Однако с его установкой вы узнаете такой термин как "температурный дрейф" в зависимости от температуры датчика  его показания могут давать существенную погрешность. Я вынужден следить в начале печати за первым слоем и оперативно компенсировать с помощью z-offset.

2. `Cartographer` - отличается наличием дополнительных модулей внутри, поэтому чуть дороже и чуть быстрее. Менее подвержен температурному дрефу но следить за первым слоем не помешает.

3. `Beacon` наиболее точный и дорогой вариант. ИМХО переплачивать нет никакой необходимости. Менее подвержен температурному дрефу но следить за первым слоем  не помешает.
...
после этого принтер перехапустится и подключаемся снова.
...
4. В зависимости от датчика и варианта его крепления команда установки будет различной. сверяемся с оригинальным руководством и в конце команды подставляем свой вариант крепления.
...
в описании крепления мы видим `X Offset = 0 Y Offset = 24.82 ` раз по оси икс оффсет ноль меньше настроек и тестов.
