...
Но не могу его рекомендовать так как работает он из рук вон плохо,  однако в сети уже есть несколько проектов которые на вид внушают доверие.  например [этот](https://www.printables.com/model/863675-nexgen3d-chamber-heater)  суть особо не меняется, у нас есть выход на 24 вольта на наш вентилятор и подключаемое реле которое коммутирует цепь 220 вольт. 
...
К сожалению править конфигурационные файлы не можем и первым делом нам придется зайти через ssh в наш принтер и вызвав меню helper script 
...
ищем макрос который начинается с **M106** удаляем либо закомментируем все строки в этом блоке.
также поступаем с макросом **M141**. сохраняем и теперь уже с перезагрузкой. если все правильно то ваша панель температур будет выглядеть как то так:
